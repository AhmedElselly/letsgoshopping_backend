...
module.exports = {
        ...
        form.parse(req, (err, fields, files) => {
            ...
            if(files.photo){
                if(files.photo.size > 1000000){
                    return res.status(400).json({error: 'The size of the image shouldn\'t be more than 1mb.'});
                }
                ...
                console.log(`files photos, ${files.photo.size}`)
    ...
    // sell / arrival
    // by sell = /api/products?sortBy=sold&order=descending&limit=4
    // by arrival = /api/products?sortBy=createdAt&order=descending&limit=4
    ...
    // find product based on request category (related products)
    // find products on same category
    listRelated(req, res, next){
        let limit = req.query.limit ? parseInt(req.query.limit) : 6;
    ...
    search(req, res, next){
        // create query object to hold search and category values
        const query = {};
